...
SECONDS=0
PACKET_SERVER_DEPLOY=0
OPENSTACK_DEPLOY_TIME=0
...
if [ ! "$SKIP_PROVISIONING" = "true" ] ; then
  ...
  if [ ! -f ${parentdir}/comparison/ansible/inventory ]; then
    ...
    if [ "$PROVISION_ONLY" = "true" ] ; then
      echo "Only provisioning resources"
      exit 0
    fi
  else
    echo "Using existing inventory from ${parentdir}/comparison/ansible/inventory"
    time docker run --rm \
      -v ${parentdir}/comparison/ansible:/ansible \
      -v ~/.ssh/id_rsa:/root/.ssh/id_rsa \
      -v ~/.ssh/id_rsa.pub:/root/.ssh/id_rsa.pub \
